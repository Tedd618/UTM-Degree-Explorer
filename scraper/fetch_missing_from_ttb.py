#!/usr/bin/env python3
"""
Fetch course info for codes present in timetable builder data
but missing from data/courses.json.

Usage:
    python3 scraper/fetch_missing_from_ttb.py
"""
...
def inject_credits_nodes(prereq_ast, prereq_raw):
    """Inject CREDITS nodes for 'minimum N credits' requirements."""
    m = re.search(r'minimum\s+of\s+([\d.]+)\s+credits?', prereq_raw, re.IGNORECASE)
    if not m:
        m = re.search(r'minimum\s+([\d.]+)\s+credits?', prereq_raw, re.IGNORECASE)
    if not m:
        return prereq_ast
    minimum = float(m.group(1))
    credits_node = {"type": "CREDITS", "minimum": minimum}
    if not prereq_ast or prereq_ast == []:
        return credits_node
    if isinstance(prereq_ast, dict):
        if prereq_ast.get('type') == 'AND':
            return {"type": "AND", "operands": [credits_node] + prereq_ast['operands']}
        return {"type": "AND", "operands": [credits_node, prereq_ast]}
    return credits_node
...
def fetch_and_parse(session, code):
    ...
    prereq_ast = inject_credits_nodes(prereq_ast, prereq_raw)
...
def main():
    fw = json.load(open(DATA_DIR / 'seasonal' / 'fall_winter_courses.json'))
    su = json.load(open(DATA_DIR / 'seasonal' / 'summer_courses.json'))
    ...
    for c in fw + su:
        ...
        if re.match(r'^[A-Z]{3}\d{3}[HY]\d$', code):
            ttb_codes.add(code)

    missing = sorted(ttb_codes - existing)
    print(f"TTB has {len(ttb_codes)} unique codes. Missing from our data: {len(missing)}")
    print("Codes to fetch:", missing)
    print()
    ...
    for i, code in enumerate(missing):
        ...
        if result:
            found.append(result)
            print(f"  [{i+1}/{len(missing)}] FOUND: {result['code']} - {result['title'][:60]}")
        else:
            print(f"  [{i+1}/{len(missing)}] NOT FOUND on calendar: {code}")
        time.sleep(0.3)

    print(f"\nFetched {len(found)} courses.")
    ...
    print(f"\nAdded {added} new courses. Total: {len(courses)}.")
